...
const DocumentVerificationSystem: React.FC<DocumentVerificationProps> = ({
  isOpen,
  onClose, 
  documentId,
  userData,
  ...
  const [activeTab, setActiveTab] = useState<'identity' | 'financial' | 'ownership' | 'authenticity' | 'all_documents' | 'credit_application' | 'compliance_check' | 'fraud_detection' | 'credit_scoring'>('identity');
  const [isVerifying, setIsVerifying] = useState(false);
  const [selectedDocument, setSelectedDocument] = useState<string | null>(null);
  const [verificationStep, setVerificationStep] = useState<'select' | 'processing' | 'results' | 'third-party-check'>('select');
  ...
  const [fraudDetectionResult, setFraudDetectionResult] = useState<FraudDetectionResult | null>(null);
  ...
  useEffect(() => {
    if (formData && Object.keys(formData).length > 3) { // More than just the defaults
  ...
  const documentTypes = [
    ...
    { id: 'accounts_receivable_aging', name: 'Accounts Receivable Aging Report', type: 'financial' }
  ];

  // Mock verification engines
  const verificationEngines = [
    { id: 'deep_doctection', name: 'DeepDoctection', description: 'Advanced document layout analysis' },
    { id: 'doc_layout_yolo', name: 'DocLayoutYOLO', description: 'Fast visual document element detection' },
    { id: 'opencv_img2table', name: 'OpenCV/img2table', description: 'Table structure extraction' },
    { id: 'scikit_classifier', name: 'Scikit-learn Classifier', description: 'Document classification' },
    { id: 'pycaret_classifier', name: 'PyCaret Classifier', description: 'Automated document classification' }
  ];
  ...
  const thirdPartyDatabases = [
    ...
    { id: 'financial', name: 'Financial Institutions', description: 'Banking and financial records' },
    ...
    if (event.target.files && event.target.files.length > 0) {
      ...
      setTimeout(() => {
        setUploadedDocuments(prev => 
          prev.map(doc => 
            doc.id === newDocument.id ? {...doc, status: 'uploaded'} : doc
          )
        );
    ...
    return (
      <div className="space-y-4">
        ...
        {thirdPartyChecks.map((check, index) => (
          <div key={index} className={`p-4 rounded-lg border ${check.verified ? 'border-green-200 bg-green-50' : 'border-orange-200 bg-orange-50'}`}>
            <div className="flex justify-between items-start mb-2">
              ...
              <div className={`px-2 py-1 rounded-full text-xs font-medium ${check.verified ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'}`}>
                {check.verified ? 'Verified' : 'Not Verified'}
              </div>
            </div>
            
            <p className="text-sm mb-2">Confidence Score: {Math.round(check.confidenceScore * 100)}%</p>
                    ...
                    {check.matchedFields.map((field, idx) => (
                      <tr key={idx}>
                        ...
                        <td className="px-3 py-2">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${field.matched ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
  ...
  const renderDocumentCard = (doc: any) => {
    ...
    const isUploaded = uploadedDocuments.some(
      uploaded => uploaded.docType === doc.type && uploaded.status === 'uploaded' && uploaded.id === selectedDocument
    );
        ...
        <div className="flex justify-between items-start">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">{doc.type.charAt(0).toUpperCase() + doc.type.slice(1)} Document</p>
      ...
      <div className="mt-4 border-t pt-4">
        ...
        <div className="space-y-2">
          {filtered.map(doc => (
            ...
            >
              <div className="flex items-center">
                <div className="p-2 bg-green-100 text-green-700 rounded-md mr-3" onClick={() => setSelectedDocument(doc.id)}>
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {Object.keys(groupedDocuments).length === 0 ? (
          <div className="text-center py-10 border border-dashed rounded-lg">
            ...
            <p className="text-gray-500 text-sm">
              Upload documents to begin verification
            </p>
          </div>
        ) : (
  ...
  const handleCreditReportUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files.length > 0) {
      ...
      const newReport: CreditReport = {
        id: `report-${Date.now()}`,
        name: file.name,
        source: file.name.includes('equifax') ? 'Equifax' : 
               file.name.includes('experian') ? 'Experian' : 
               file.name.includes('transunion') ? 'TransUnion' : 'Unknown',
  ...
  const runComplianceCheck = () => {
    ...
    setTimeout(() => {
      const result: ComplianceCheckResult = {
        status: Math.random() > 0.7 ? 'compliant' : Math.random() > 0.5 ? 'non-compliant' : 'needs-review',
        regulations: [
          {
  ...
  const runFraudDetection = () => {
    ...
    setTimeout(() => {
      const result: FraudDetectionResult = {
        ...
        verificationStatus: Math.random() > 0.7 ? 'verified' : Math.random() > 0.3 ? 'suspicious' : 'unverified',
        lastChecked: new Date().toISOString()
      };
          ...
          <div className="p-4 bg-blue-50 rounded-lg">
            ...
            {creditReports.length > 0 ? (
                        ...
                        {report.status === 'analyzed' && report.score && (
                          <div className="flex items-center space-x-2">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                              report.score > 700 ? 'bg-green-100 text-green-800' : 
                              report.score > 600 ? 'bg-yellow-100 text-yellow-800' : 
                              'bg-red-100 text-red-800'
                            }`}>
                              Score: {report.score}
                            </span>
              ...
              <div className="text-center py-8 border border-dashed rounded-lg">
                ...
                <p className="text-sm text-gray-500 mt-1">Upload PDF credit reports to see analysis</p>
              </div>
            )}
          </div>
            ...
            <div className="p-4 bg-green-50 rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                {['Equifax', 'Experian', 'TransUnion'].map(bureau => {
                  const report = creditReports.find(r => r.source === bureau && r.status === 'analyzed');
                  return (
                      ...
                      {report ? (
                        <div className="mt-2">
                          <p className={`text-xl font-bold ${
                            (report.score || 0) > 700 ? 'text-green-600' : 
                            (report.score || 0) > 600 ? 'text-yellow-600' : 
                            'text-red-600'
                          }`}>
                            {report.score}
                          </p>
                      ...
                      return (
                        <div 
                          className={`h-2.5 rounded-full ${
                            avgScore > 700 ? 'bg-green-600' : 
                            avgScore > 600 ? 'bg-yellow-600' : 
                            'bg-red-600'
                          }`} 
        ...
        {complianceResult ? (
          <div className="space-y-6">
            ...
            <div className={`p-4 rounded-lg ${
              complianceResult.status === 'compliant' ? 'bg-green-50' :
              complianceResult.status === 'non-compliant' ? 'bg-red-50' :
              'bg-yellow-50'
            }`}>
              <div className="flex items-center mb-3">
                <div className={`rounded-full p-2 mr-3 ${
                  complianceResult.status === 'compliant' ? 'bg-green-100 text-green-600' :
                  complianceResult.status === 'non-compliant' ? 'bg-red-100 text-red-600' :
                ...
                <div>
                  <h4 className="font-medium text-base capitalize">
                    <span>{complianceResult.status === 'compliant' ? 'Compliant' : complianceResult.status === 'non-compliant' ? 'Non-Compliant' : 'Needs Review'}</span>
                  </h4>
                ...
                {complianceResult.regulations.map((reg, index) => (
                  <div 
                    ...
                    className={`p-3 rounded-lg border ${
                      reg.status === 'pass' ? 'border-green-200 bg-green-50' :
                      reg.status === 'fail' ? 'border-red-200 bg-red-50' :
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        reg.status === 'pass' ? 'bg-green-100 text-green-800' :
                        reg.status === 'fail' ? 'bg-red-100 text-red-800' :
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        ...
        {fraudDetectionResult ? (
          <div className="space-y-6">
            ...
            <div className={`p-4 rounded-lg ${
              fraudDetectionResult.riskLevel === 'low' ? 'bg-green-50' :
              fraudDetectionResult.riskLevel === 'medium' ? 'bg-yellow-50' :
              'bg-red-50'
            }`}>
              <div className="flex items-center">
                <div className={`rounded-full p-2 mr-3 ${
                  fraudDetectionResult.riskLevel === 'low' ? 'bg-green-100 text-green-600' :
                  fraudDetectionResult.riskLevel === 'medium' ? 'bg-yellow-100 text-yellow-600' :
                ...
                <div>
                  <h4 className="font-medium text-base capitalize">
                    <span>{fraudDetectionResult.riskLevel === 'low' ? 'Low Risk' : fraudDetectionResult.riskLevel === 'medium' ? 'Medium Risk' : 'High Risk'}</span> - 
                    <span className="ml-1 capitalize">{fraudDetectionResult.verificationStatus}</span>
                  </h4>
                  <p className="text-sm text-gray-600">
              ...
              <div className="space-y-3">
                {fraudDetectionResult.alertFlags.map((flag, index) => (
                  <div 
                    key={index}
                    className={`p-3 rounded-lg border ${
                      flag.severity === 'low' ? 'border-green-200 bg-green-50' :
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        flag.severity === 'low' ? 'bg-green-100 text-green-800' :
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="annualRevenue" className="block text-sm font-medium text-gray-700">
                  {formData.applicantType === 'business' ? 'Annual Business Revenue*' : 'Annual Income*'}
                </label>
                <div className="mt-1 relative rounded-md shadow-sm">
                  ...
                  <input
                    ...
                    value={formData.applicantType === 'business' ? (formData.annualRevenue || '') : (formData.annualIncome || '')}
                    onChange={handleInputChange}
              ...
              <div>
                <label htmlFor="creditScore" className="block text-sm font-medium text-gray-700">Credit Score (if known)</label>
                <input
                  type="text"
                  id="creditScore"
                  name="creditScore"
              ...
              <div>
                <label htmlFor="bankName" className="block text-sm font-medium text-gray-700">Primary Bank</label>
                <input
                  type="text"
                  id="bankName"
                  name="bankName"
          ...
          <div className="p-4 bg-green-50 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="loanAmount" className="block text-sm font-medium text-gray-700">Requested Amount*</label>
                <div className="mt-1 relative rounded-md shadow-sm">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <span className="text-gray-500 sm:text-sm">$</span>
                  </div>
              ...
              <div>
                <label htmlFor="loanPurpose" className="block text-sm font-medium text-gray-700">Purpose of Financing*</label>
                <select
                  id="loanPurpose"
                  name="loanPurpose"
              ...
              <div>
                <label htmlFor="loanTerm" className="block text-sm font-medium text-gray-700">Preferred Term*</label>
                <select
                  id="loanTerm"
                  name="loanTerm"
              ...
              <div>
                <label htmlFor="collateral" className="block text-sm font-medium text-gray-700">Collateral (if applicable)</label>
                <input
                  type="text"
                  id="collateral"
                  name="collateral"
          ...
          <div className="p-4 bg-indigo-50 rounded-lg">
            ...
            <p className="text-sm text-gray-600 mb-4">
              By signing below, I certify that all information provided is accurate and authorize EVA AI to obtain credit reports
              and verify information provided for the purpose of evaluating this credit application.
            </p>
            ...
            <div className="flex items-start">
              ...
              <label htmlFor="agreeToTerms" className="ml-2 text-sm text-gray-700">
                ...
                <a href="#" className="text-indigo-600 hover:text-indigo-800"> Privacy Policy</a>, and consent to the 
                electronic processing of my application.
              </label>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
      ...
      >
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-semibold">{documentId ? 'Verify Document' : 'New Credit Application'}</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-col md:flex-row">
          <div className="md:w-1/2">
            ...
            <div className="mb-6">
              ...
              {/* Document Selection */}
              <div className="bg-white p-4 rounded-lg border border-gray-200">
                <h3 className="text-lg font-medium text-gray-800 mb-2">Select Document to Verify</h3>
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <button
                    onClick={() => { setActiveTab('all_documents'); setShowDocumentManager(true); }}
              ...
              <div className="bg-white p-4 rounded-lg border border-gray-200">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start pt-2">
                    ...
                    />
                    <div className="ml-2">
                      <label htmlFor="enhanced-verification" className="text-sm font-medium">Enable Enhanced Verification</label>
                      <p className="text-xs text-gray-500">Uses multiple verification engines and cross-checks results with third-party databases</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {verificationStep === 'processing' && (
              <div className="bg-white p-8 rounded-lg border border-gray-200 text-center">
                ...
                <p className="text-gray-500">This may take a moment while we analyze the document...</p>
              </div>
            )}
              ...
              <div className={`bg-white p-4 rounded-lg border ${
                ...
                <div className="flex items-center mb-4">
                  <div className={`p-2 rounded-full ${
                    verificationResults.verified ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'
                  }`}>
                  ...
                  <div className="ml-3">
                    <p className="text-lg font-medium">
                      {verificationResults.verified ? 'Document Verified' : 'Verification Issues Found'}
                    </p>
                    <p className="text-sm text-gray-600">
            ...
            {!showDocumentManager && activeTab !== 'credit_application' && (
              <button
                ...
                className={`px-4 py-2 text-sm text-white rounded-md ${
                  isVerifying || (verificationStep === 'select' && !selectedDocument) ? 'bg-primary-400 cursor-not-allowed' : 'bg-primary-600 hover:bg-primary-700'
                }`}
              >
                {isVerifying ? (
