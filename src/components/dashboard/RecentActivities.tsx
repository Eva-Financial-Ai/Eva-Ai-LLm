...
export const RecentActivities: React.FC<RecentActivitiesProps> = ({ activities }) => {
  ...
  const getUserTypeLabel = (type: UserType): string => {
    switch (type) {
      case UserType.BUSINESS: return 'Borrower';
      case UserType.VENDOR: return 'Vendor';
      case UserType.BROKERAGE: return 'Broker';
      case UserType.LENDER: return 'Lender';
      default: return 'User';
    }
  };
  ...
  const getUserTypeColor = (type: UserType): string => {
    switch (type) {
      case UserType.BUSINESS: return 'bg-blue-100 text-blue-800';
      case UserType.VENDOR: return 'bg-green-100 text-green-800';
      case UserType.BROKERAGE: return 'bg-purple-100 text-purple-800';
      case UserType.LENDER: return 'bg-indigo-100 text-indigo-800';
              ...
              <div className="flex-1 space-y-1">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getUserTypeColor(activity.userType)}`}>
