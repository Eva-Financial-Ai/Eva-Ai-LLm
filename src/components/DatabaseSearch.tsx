    ...
    <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
      ...
      <div className="flex gap-2 mb-4">
        <div className="flex-grow">
          <input
            ...
            placeholder={searchType === 'business' ? "Search by business name, DBA, or Tax ID" : "Search by owner name or SSN"}
      ...
      {!isSearching && searchPerformed && searchResults.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          No {searchType === 'business' ? 'businesses' : 'owners'} found matching your search criteria.
        </div>
      )}
                  ...
                  {searchType === 'business' ? (
                    <>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="font-medium text-gray-900">{(item as BusinessRecord).businessName}</div>
                        {(item as BusinessRecord).dba && (
                          <div className="text-sm text-gray-500">DBA: {(item as BusinessRecord).dba}</div>
                        )}
                      </td>
