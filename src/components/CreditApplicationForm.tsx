...
import TaxReturnsUpload, { BusinessTaxReturnsState, OwnerTaxReturnsState } from './tax/TaxReturnsUpload';
...
const CreditApplicationForm: React.FC<CreditApplicationFormProps> = ({ 
  ...
  const initFormData = (): FormData => {
    const formBusinessType = localBusinessType === 'existing' ? 'existing_business' : 'new_business';
  ...
  const [businessAddressInput, setBusinessAddressInput] = useState(initialData.businessAddress || '');
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    // Format money fields
    else if (['requestedAmount', 'annualRevenue', 'monthlyExpenses', 'outstandingDebt', 'cashBalance'].includes(name)) {
  ...
  const validateForm = () => {
    ...
    } else if (!isValidEin(formData.taxId)) {
    ...
    if ((formData.dba || formData.businessType === 'sole_proprietorship') && !formData.countyOfDba) {
      newErrors.countyOfDba = 'County of Doing Business As is required when DBA is provided or business type is Sole Proprietorship';
    }
    ...
    if (!formData.owners || formData.owners.length === 0 || !formData.owners[0]) {
      ...
      } else if (parseFloat(primaryOwner.ownershipPercentage) < 0 || parseFloat(primaryOwner.ownershipPercentage) > 100) {
        newErrors.ownershipPercentage = 'Ownership percentage must be between 0 and 100';
      } else if (parseFloat(primaryOwner.ownershipPercentage) < 81 && formData.owners.length === 1) {
        newErrors.ownershipPercentage = 'Primary owner has less than 81% ownership. Additional owners must be identified for compliance.';
      }
      
      // Check that total ownership percentage equals 100%
      const totalOwnership = formData.owners.reduce((sum, owner) => sum + (parseFloat(owner.ownershipPercentage) || 0), 0);
      if (Math.abs(totalOwnership - 100) > 0.01) { // Allow small rounding errors
    ...
    } else if (isNaN(Number(formData.requestedAmount.replace(/,/g, ''))) || Number(formData.requestedAmount.replace(/,/g, '')) <= 0) {
    ...
    } else if (isNaN(Number(formData.annualRevenue.replace(/,/g, ''))) || Number(formData.annualRevenue.replace(/,/g, '')) < 0) {
    ...
    if (formData.monthlyExpenses && (isNaN(Number(formData.monthlyExpenses.replace(/,/g, ''))) || Number(formData.monthlyExpenses.replace(/,/g, '')) < 0)) {
      newErrors.monthlyExpenses = 'Please enter a valid amount';
    } else if (formData.monthlyExpenses && Number(formData.monthlyExpenses.replace(/,/g, '')) > 150000000000) {
      newErrors.monthlyExpenses = 'Maximum value is $150 billion';
    }
    
    // Outstanding debt validation
    if (formData.outstandingDebt && (isNaN(Number(formData.outstandingDebt.replace(/,/g, ''))) || Number(formData.outstandingDebt.replace(/,/g, '')) < 0)) {
      newErrors.outstandingDebt = 'Please enter a valid amount';
    } else if (formData.outstandingDebt && Number(formData.outstandingDebt.replace(/,/g, '')) > 150000000000) {
      newErrors.outstandingDebt = 'Maximum value is $150 billion';
    }
    
    // Cash balance validation
    if (formData.cashBalance && (isNaN(Number(formData.cashBalance.replace(/,/g, ''))) || Number(formData.cashBalance.replace(/,/g, '')) < 0)) {
      newErrors.cashBalance = 'Please enter a valid amount';
    } else if (formData.cashBalance && Number(formData.cashBalance.replace(/,/g, '')) > 150000000000) {
      newErrors.cashBalance = 'Maximum value is $150 billion';
    }
    
    // Gross profit margin validation
    if (formData.grossProfitMargin && (isNaN(Number(formData.grossProfitMargin)) || Number(formData.grossProfitMargin) < 0)) {
      newErrors.grossProfitMargin = 'Please enter a valid percentage';
    } else if (formData.grossProfitMargin && Number(formData.grossProfitMargin) > 9000 && !formData.highProfitMarginAcknowledged) {
  ...
  const openPlaidLink = () => {
    ...
    setTimeout(() => {
      ...
      const mockAccounts: PlaidAccount[] = [
        { id: 'acc1', name: 'Business Checking', type: 'checking', balance: 25430.21, institution: 'Chase' },
        { id: 'acc2', name: 'Business Savings', type: 'savings', balance: 158750.82, institution: 'Chase' },
        { id: 'acc3', name: 'Business Line of Credit', type: 'credit', balance: -15000, institution: 'Chase' }
      ];
  ...
  const handleAddressConfirm = () => {
    ...
    if (isBusinessAddress) {
      setFormData(prev => ({
        ...
        businessState: addressToVerify.state,
      ...
      setFormData(prev => ({
        ...
        ownerState: addressToVerify.state,
  ...
  const handleZipCodeChange = (e: React.ChangeEvent<HTMLInputElement>, isBusinessAddress: boolean) => {
  ...
  const handleBusinessSelection = (business: BusinessRecord) => {
    ...
    setFormData(prevData => {
      ...
      return {
        ...
        owners: business.owners?.length ? business.owners.map(o => ({
          ...newOwner,
          id: o.id,
          name: o.name,
  ...
  if (submitted && transactionId) {
    return (
      <div className="bg-green-50 p-6 rounded-lg text-center border border-green-200">
        ...
        <p className="mt-2 text-green-600">Your application has been successfully submitted and is being processed.</p>
  ...
  const handleAccountingConnect = useCallback((type: 'quickbooks' | 'netsuite') => {
    setTimeout(() => {
      ...
      if (type === 'quickbooks') {
        mockStatements.push(
          { id: 'qb-pl-' + (currentYear-1), name: `Profit & Loss ${currentYear-1}`, source: 'quickbooks', type: 'profit_loss', year: String(currentYear-1), url: '#' },
          { id: 'qb-pl-' + (currentYear-2), name: `Profit & Loss ${currentYear-2}`, source: 'quickbooks', type: 'profit_loss', year: String(currentYear-2), url: '#' },
          { id: 'qb-bs-' + (currentYear-1), name: `Balance Sheet ${currentYear-1}`, source: 'quickbooks', type: 'balance_sheet', year: String(currentYear-1), url: '#' },
          { id: 'qb-bs-' + (currentYear-2), name: `Balance Sheet ${currentYear-2}`, source: 'quickbooks', type: 'balance_sheet', year: String(currentYear-2), url: '#' }
        );
      } else {
        mockStatements.push(
          { id: 'ns-pl-' + (currentYear-1), name: `Profit & Loss ${currentYear-1}`, source: 'netsuite', type: 'profit_loss', year: String(currentYear-1), url: '#' },
          { id: 'ns-pl-' + (currentYear-2), name: `Profit & Loss ${currentYear-2}`, source: 'netsuite', type: 'profit_loss', year: String(currentYear-2), url: '#' },
          { id: 'ns-pl-' + (currentYear-3), name: `Profit & Loss ${currentYear-3}`, source: 'netsuite', type: 'profit_loss', year: String(currentYear-3), url: '#' },
          { id: 'ns-bs-' + (currentYear-1), name: `Balance Sheet ${currentYear-1}`, source: 'netsuite', type: 'balance_sheet', year: String(currentYear-1), url: '#' },
          { id: 'ns-bs-' + (currentYear-2), name: `Balance Sheet ${currentYear-2}`, source: 'netsuite', type: 'balance_sheet', year: String(currentYear-2), url: '#' }
        );
      }
  ...
  const handleAccountingDisconnect = useCallback((type: 'quickbooks' | 'netsuite') => {
    setFormData({
      ...formData,
  ...
  const handleFinancialStatementsUpload = useCallback((files: FileList) => {
    ...
    const newStatements = Array.from(files).map(file => ({
      ...
      type: file.name.toLowerCase().includes('balance') ? 'balance_sheet' as const : 'profit_loss' as const,
  ...
  const handleBusinessTaxReturnsChange = useCallback((data: BusinessTaxReturnsState) => {
    setFormData({
      ...formData,
  ...
  const handleOwnerTaxReturnsChange = useCallback((ownerId: string, data: Partial<OwnerTaxReturnsState>) => {
  ...
  const handleUploadBusinessTaxReturn = useCallback((files: FileList) => {
    // In a real app, this would upload files to the server
    // Here we're just creating mock entries
  ...
  const handleUploadOwnerTaxReturn = useCallback((ownerId: string, files: FileList) => {
    // In a real app, this would upload files to the server
    // Here we're just creating mock entries
    const newFiles = Array.from(files).map(file => ({
      id: uuidv4(),
  ...
  const handleAddressInputChange = (e: React.ChangeEvent<HTMLInputElement>, isBusinessAddress: boolean) => {
    const { value } = e.target;
      ...
      <>
              ...
              <ul className="divide-y divide-gray-200">
                {businessAddressSuggestions.map(suggestion => (
                  ...
                  >
                    ...
                    <div className="text-gray-500 text-xs">{suggestion.city}, {suggestion.state} {suggestion.zipCode}</div>
        ...
        <div>
          ...
          <input
            ...
            required
          />
          {errors.businessCity && <p className="mt-1 text-sm text-red-600">{errors.businessCity}</p>}
        </div>
        ...
        <div>
          ...
          {errors.businessState && <p className="mt-1 text-sm text-red-600">{errors.businessState}</p>}
        </div>
        ...
        <div>
          ...
          {formData.businessZip && zipCodeConfirm && formData.businessZip !== zipCodeConfirm && !zipCodeError && !addressVerified && (
            <p className="mt-1 text-sm text-yellow-600">ZIP codes don't match. Please verify your address.</p>
          )}
          {addressVerified && (
            <p className="mt-1 text-sm text-green-600">Address verified successfully. You can proceed to the next step.</p>
          )}
        </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 divide-y divide-gray-200">
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm text-green-700">
                    <span className="font-medium">Pre-filled data:</span> Using existing business information for <strong>{initialData.businessName}</strong>. The "Existing Business" option has been automatically selected.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4">
              <div 
                className={`flex-1 p-4 border rounded-lg cursor-pointer ${
                  localBusinessType === 'new' ? 'border-primary-500 bg-primary-50' : 'border-gray-300 hover:bg-gray-50'
                }`}
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">I'm entering information for a new business not yet in the system</p>
                  </div>
                  <div className={`h-5 w-5 rounded-full border flex items-center justify-center ${
                    localBusinessType === 'new' ? 'border-primary-500 bg-primary-100' : 'border-gray-300'
                  }`}>
              ...
              <div 
                className={`flex-1 p-4 border rounded-lg cursor-pointer ${
                  localBusinessType === 'existing' ? 'border-primary-500 bg-primary-50' : 'border-gray-300 hover:bg-gray-50'
                }`}
                onClick={() => {
                  setLocalBusinessType('existing');
                  setShowDatabaseSearch(true);
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">Use information from an existing business in our database</p>
                  </div>
                  <div className={`h-5 w-5 rounded-full border flex items-center justify-center ${
                    localBusinessType === 'existing' ? 'border-primary-500 bg-primary-100' : 'border-gray-300'
                  }`}>
          ...
          <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
            ...
            <div className="mb-4">
              <p className="text-sm text-gray-500">
                Please provide information about all owners with 20% or greater ownership interest in the business.
              </p>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Requested Amount */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="requestedAmount">
                  Requested Amount *
                </label>
                <div className="relative">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="financialInstrument">
                  Financial Instrument *
                </label>
                <select
                ...
                {errors.financialInstrument && <p className="mt-1 text-sm text-red-600">{errors.financialInstrument}</p>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Annual Revenue */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="annualRevenue">
                  Annual Revenue *
                </label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              ...
              {/* Monthly Expenses */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="monthlyExpenses">
                  Monthly Expenses
                </label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                ...
                {errors.monthlyExpenses && <p className="mt-1 text-sm text-red-600">{errors.monthlyExpenses}</p>}
              </div>
            </div>
          </div>
        </div>
      ...
      />
          ...
          {/* Lien & UCC Management */}
          <LienUCCManagement 
            businessId={formData.businessName ? formData.businessName.toLowerCase().replace(/\s+/g, '-') : undefined}
          ...
          {currentStep < totalSteps ? (
            <button
              ...
              className={`inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white ${
                currentStep === 1 && !addressVerified ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500'
              }`}
            >
