...
import { Asset, AssetClass, AssetClassNames, VerificationStatus, PortfolioUserRole } from '../../types/AssetClassTypes';
...
export const AssetCardGrid: React.FC<AssetCardGridProps> = ({ 
  ...
  const uniqueAssetClasses = Array.from(
    new Set(assets.map(asset => asset.assetClass))
  ).sort();
      ...
      case AssetClass.CRYPTO:
        return (
          <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 14l-2 2m0 0l-2-2m2 2V9" />
          </svg>
        );
      default:
        return (
    ...
    <div className={`space-y-4 ${className}`}>
          ...
          <div className="ml-2">
            ...
            >
              ...
              {uniqueAssetClasses.map(assetClass => (
                <option key={assetClass} value={assetClass}>{AssetClassNames[assetClass as AssetClass]}</option>
              ))}
            </select>
          </div>
        </div>
          ...
          >
            <div className="p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center">
                  <div className={`p-2 rounded-lg ${asset.performance && asset.performance > 0 ? 'bg-green-100' : 'bg-red-100'}`}>
                ...
                <div className={`px-2 py-1 text-xs rounded-full ${getVerificationStatusColor(asset.blockchainVerification.status)}`}>
                  {asset.blockchainVerification.status.charAt(0).toUpperCase() + asset.blockchainVerification.status.slice(1)}
                </div>
              </div>
              
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-lg font-semibold">{formatCurrency(asset.financialData.marketValue)}</p>
                  {asset.performance && (
                    <div className={`text-xs font-medium ${asset.performance > 0 ? 'text-green-600' : 'text-red-600'}`}>
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs font-medium">{new Date(asset.lastUpdate).toLocaleDateString()}</p>
                </div>
              </div>
            </div>
              ...
              <div className="flex border-t border-gray-200 divide-x divide-gray-200">
                {asset.blockchainVerification.status !== VerificationStatus.VERIFIED && (
                  <button className="flex-1 py-2 text-xs font-medium text-primary-600 hover:bg-primary-50">
                    {asset.blockchainVerification.status === VerificationStatus.PENDING ? 'Check Status' : 'Verify Asset'}
                  </button>
                )}
                {asset.blockchainVerification.status === VerificationStatus.VERIFIED && (
      ...
      {filteredAssets.length === 0 && (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          <p className="mt-1 text-sm text-gray-500">Try adjusting your filters or add new assets to your portfolio.</p>
