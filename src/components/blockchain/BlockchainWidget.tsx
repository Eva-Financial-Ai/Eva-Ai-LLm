...
const BlockchainWidget: React.FC<BlockchainWidgetProps> = ({ initialVisible = false }) => {
  ...
  const commercialInstruments = [
    ...
    { id: 2, name: 'Treasury Bills', symbol: 'TB', price: 995.50, change: -0.10 },
    { id: 3, name: 'Corporate Bonds', symbol: 'CB', price: 102.25, change: +1.15 },
    { id: 4, name: 'Municipal Bonds', symbol: 'MB', price: 105.50, change: +0.45 },
    { id: 5, name: 'Certificate of Deposit', symbol: 'CD', price: 100.00, change: 0.00 }
  ];
  ...
  const portfolio = {
    ...
    percentChange: 2.35,
    instruments: [
      { id: 1, name: 'Commercial Paper', symbol: 'CP', balance: 50000, value: 49375.00 },
      { id: 3, name: 'Corporate Bonds', symbol: 'CB', balance: 1000, value: 102250.00 },
  ...
  const recentTransactions = [
    { id: 't1', type: 'buy', instrument: 'Commercial Paper', symbol: 'CP', amount: 10000, price: 98.50, date: '2023-10-25T14:30:00' },
    { id: 't2', type: 'sell', instrument: 'Treasury Bills', symbol: 'TB', amount: 5000, price: 995.75, date: '2023-10-24T09:15:00' },
    { id: 't3', type: 'buy', instrument: 'Corporate Bonds', symbol: 'CB', amount: 200, price: 101.10, date: '2023-10-22T11:45:00' }
  ];
  ...
  const MinimizedView = () => (
    <div 
      className="w-full h-full rounded-full bg-indigo-600 flex items-center justify-center cursor-pointer transition-all hover:bg-indigo-700"
      ...
      <div className="bg-indigo-700 text-white p-4">
        <div className="flex justify-between items-center">
          ...
          <div className="text-right">
            ...
            <p className={`text-lg font-bold ${portfolio.percentChange >= 0 ? 'text-green-300' : 'text-red-300'}`}>
              {portfolio.percentChange >= 0 ? '+' : ''}{portfolio.percentChange}%
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2">
            {portfolio.instruments.map(asset => (
              <div key={asset.id} className="flex justify-between items-center p-2 bg-gray-50 rounded-md">
                <div>
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-500">{(asset.value / portfolio.totalValue * 100).toFixed(1)}% of portfolio</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {recentTransactions.map(tx => (
              <div key={tx.id} className="flex justify-between items-center p-2 bg-gray-50 rounded-md">
                <div className="flex items-center">
                  <div className={`rounded-full w-8 h-8 flex items-center justify-center mr-2 ${tx.type === 'buy' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                    {tx.type === 'buy' ? (
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="text-right">
                  <p className="text-sm font-medium">{formatCurrency(tx.price * tx.amount / 100)}</p>
  ...
  const FullAppView = () => {
    ...
    const handleExecuteContract = useCallback(async (contractId: string, functionName: string, params: any) => {
      try {
    ...
    return (
      <div className="w-full h-full flex flex-col bg-white">
        ...
        <div className="px-6 py-4 bg-gray-800 text-white flex items-center justify-between">
          <div className="flex items-center">
            <svg className="w-8 h-8 mr-2 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
              ...
              <div className="lg:col-span-1 space-y-6">
                <div className="bg-white rounded-lg shadow-md p-4">
                  ...
                  <div className="mb-4">
                    ...
                    <p className={`text-lg font-medium ${portfolio.percentChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {portfolio.percentChange >= 0 ? '+' : ''}{portfolio.percentChange}%
                    </p>
                  </div>
                    ...
                    <div className="mt-2 space-y-1">
                      {portfolio.instruments.map((instrument, index) => (
                        <div key={instrument.id} className="flex items-center text-xs">
                          <div className={`w-3 h-3 mr-1 rounded-sm ${
                            index % 3 === 0 ? 'bg-indigo-500' : 
                            index % 3 === 1 ? 'bg-blue-500' : 
                            'bg-purple-500'
                          }`}></div>
                          <span>{instrument.name}: {(instrument.value / portfolio.totalValue * 100).toFixed(1)}%</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-lg shadow-md p-4">
                  ...
                  <div className="mb-4">
                    ...
                    <input type="number" className="w-full p-2 border border-gray-300 rounded-md" placeholder="Enter amount" min="0" />
                  </div>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b">
                          <th className="px-4 py-2 text-left text-gray-500 font-medium text-sm">Name</th>
                          <th className="px-4 py-2 text-right text-gray-500 font-medium text-sm">Price</th>
                          <th className="px-4 py-2 text-right text-gray-500 font-medium text-sm">24h Change</th>
                          <th className="px-4 py-2 text-right text-gray-500 font-medium text-sm">Market Cap</th>
                          <th className="px-4 py-2 text-right text-gray-500 font-medium text-sm">Actions</th>
                        </tr>
                      </thead>
                      <tbody>
                        {commercialInstruments.map(instrument => (
                  ...
                  <div className="space-y-3">
                    {recentTransactions.map(tx => (
                      <div key={tx.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-md">
                        <div className="flex items-center">
                          <div className={`rounded-full w-10 h-10 flex items-center justify-center mr-3 ${tx.type === 'buy' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                            {tx.type === 'buy' ? (
                        ...
                        <div className="text-right">
                          <p className="font-medium">{formatCurrency(tx.price * tx.amount / 100)}</p>
  ...
  if (!isVisible) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 w-12 h-12 rounded-full bg-indigo-600 shadow-lg flex items-center justify-center text-white hover:bg-indigo-700 focus:outline-none z-50"
      >
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
  ...
  return (
    <animated.div
      ...
      className={`bg-white shadow-2xl ${widgetState !== 'full' ? 'shadow-lg' : ''}`}
    >
