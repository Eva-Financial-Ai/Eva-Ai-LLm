...
const AI_MODELS = [
  ...
  { id: 'deal-structuring', name: 'Deal Structuring Expert' },
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      switch (resizeDirection) {
        case 'nw':
          ...
          newWidth = Math.max(300, resizeStartRef.current.width - (e.clientX - resizeStartRef.current.x));
          newHeight = Math.max(300, resizeStartRef.current.height - (e.clientY - resizeStartRef.current.y));
...
Would you like to view a summary of the orchestrated data or proceed with analysis?`,
  ...
  const getAvailableDataSources = () => {
    return Object.values(collectionMethods).map(method => 
  ...
  const sendMessage = () => {
    ...
    if ((messageText.includes('document verification') || messageText.includes('verify document')) && selectedModel.id === 'document-verification') {
    ...
    } else if ((messageText.includes('data orchestration') || messageText.includes('orchestrate data')) && selectedModel.id === 'data-orchestration') {
  ...
  const getAIResponse = (userInput: string, modelId: string): string => {
    ...
    if (input.includes('financial statement')) {
      return modelPrefix + "For financial statements, we typically need the past 2 years of statements. These should include the income statement, balance sheet, and cash flow statement. Would you like me to analyze them when they're uploaded?";
    } else if (input.includes('document') && !input.includes('verify')) {
      return modelPrefix + "For an equipment loan, we require: business financial statements, tax returns (last 2 years), bank statements (3 months), and the equipment invoice or quote. All documents should be in PDF format.";
    } else if (input.includes('credit')) {
      return modelPrefix + "I can analyze credit profiles once uploaded to the system. We typically look at personal credit scores above 680 for the best rates, but can work with scores as low as 600 with additional documentation.";
    } else if (input.includes('rate')) {
      return modelPrefix + "Current rates for equipment loans range from 5.99% to 9.99% depending on credit profile, time in business, and equipment type. For this specific transaction type, most approved applications are seeing rates around 7.25%.";
    } else if (input.includes('structure') || input.includes('deal')) {
      return modelPrefix + "Based on this transaction profile, I'd recommend a 60-month term with a 10% down payment. This optimizes monthly cash flow while maintaining an approval-friendly structure. Would you like me to prepare this deal structure for submission?";
    } else if (input.includes('analysis') || input.includes('sufficient')) {
      return modelPrefix + "Based on the documents you've uploaded so far (2 of 4 required), you still need to provide the tax returns and equipment invoice. The financial statements you provided show good cash flow, which is positive for approval. Would you like me to help you prepare the remaining documents?";
    } else if (fileUploads.length > 0) {
      return modelPrefix + `I've received your file${fileUploads.length > 1 ? 's' : ''} (${fileUploads.map(f => f.name).join(', ')}). Would you like me to analyze ${fileUploads.length > 1 ? 'these documents' : 'this document'} in the context of your ${currentTransaction?.type || 'application'}?`;
    } else {
      return modelPrefix + `I'm analyzing this ${currentTransaction?.type || 'transaction'} based on your question. To provide more specific guidance, could you share more details about your specific needs? I can help with document preparation, credit analysis, deal structuring, or approval optimization.`;
    }
  };
      ...
      {isOpen && (
        <div 
          ...
          className={`fixed z-20 transition-all duration-300 shadow-xl rounded-lg overflow-hidden ${
            minimized 
              ? 'bottom-8 right-8 w-96 h-20 bg-primary-600' 
              : ''
          }`}
          style={{
