...
const DocumentList = () => {
  ...
  const getRequiredDocuments = (): Document[] => {
    ...
    if (currentTransaction.type === 'Finance Lease' || currentTransaction.type === 'Equipment Loan') {
      baseDocuments.push({
        id: 'doc-004',
  ...
  const getStatusBadge = (status: Document['status']) => {
    switch (status) {
      case 'required':
        return <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Required</span>;
      case 'uploaded':
        return <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Uploaded</span>;
      case 'verified':
          ...
          <div>
            ...
            {documents.every(d => d.status === 'uploaded' || d.status === 'verified') ? (
              ...
              <span className="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                <svg className="-ml-0.5 mr-1 h-3 w-3 text-yellow-500" fill="currentColor" viewBox="0 0 8 8">
                  <circle cx="4" cy="4" r="3" />
                </svg>
                {documents.length - documents.filter(d => d.status === 'uploaded' || d.status === 'verified').length} more required
              </span>
            )}
          </div>
              ...
              {viewingDocument.fileURL ? (
                ...
                <div className="flex items-center justify-center h-full">
                  <div className="text-center p-8">
                    ...
                    <p className="text-sm text-gray-400 mt-2">This is a sample document viewer. In a real implementation, the actual document would be displayed here.</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex justify-between items-center mt-4">
              <div>
                <span className="text-sm text-gray-500">
                  Uploaded on {viewingDocument.uploadedAt ? new Date(viewingDocument.uploadedAt).toLocaleDateString() : 'Unknown date'}
                </span>
              </div>
              <div className="flex space-x-2">
                <button
