...
import { TermSheetDocument } from '../../services/DocumentGenerationService';
import DocumentTrackingService, { DocumentStatusUpdate, DocumentTrackingResult } from '../../services/DocumentTrackingService';
...
const DocumentStatusViewer: React.FC<DocumentStatusViewerProps> = ({ documentId, onClose }) => {
  ...
  const { document, statusUpdates, isFundingEligible, pendingItems, completedItems } = trackingResult;
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {document.participants.map((participant) => (
                <tr key={participant.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900 capitalize">{participant.role.replace(/_/g, ' ')}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadgeColor(participant.signatureStatus || 'pending')}`}>
                      {(participant.signatureStatus || 'pending').replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadgeColor(participant.kycStatus || 'pending')}`}>
                      {(participant.kycStatus || 'pending').replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {participant.role !== 'borrower' ? (
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadgeColor(participant.kybStatus || 'pending')}`}>
                        {(participant.kybStatus || 'pending').replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                      </span>
                    ) : (
            ...
            <div className="space-y-3">
              {statusUpdates.map((update, index) => (
                <div key={index} className="flex items-start">
                  <div className={`${
                    update.updateType === 'signature' ? 'bg-blue-100' :
                    update.updateType === 'kyc' ? 'bg-purple-100' :
                    update.updateType === 'kyb' ? 'bg-indigo-100' :
                    'bg-gray-100'
