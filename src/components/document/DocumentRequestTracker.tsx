...
const DocumentRequestTracker: React.FC<DocumentRequestTrackerProps> = ({
  ...
  const [documentRequests, setDocumentRequests] = useState<DocumentRequest[]>([
    {
      ...
      documents: [
        { id: 'doc-001', name: 'Business Financial Statements', description: 'Last 2 years of financial statements' },
    ...
    },
    {
      ...
      documents: [
        { id: 'doc-004', name: 'Equipment Invoice/Quote', description: 'Detailed invoice for equipment' },
  ...
  const addDocumentRequest = (newRequest: Omit<DocumentRequest, 'id' | 'status' | 'sentAt' | 'completionPercentage'>) => {
  ...
  const getStatusBadge = (status: DocumentRequest['status']) => {
    switch (status) {
      ...
      case 'partially_completed':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">Partially Completed</span>;
      case 'completed':
        ...
        <div className="overflow-hidden overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Requested Documents</th>
                      ...
                      {request.recipients.map(recipient => (
                        <div key={recipient.id} className="flex items-center">
                          <span className={`inline-flex mr-2 h-2 w-2 rounded-full ${
                            recipient.role === 'owner' ? 'bg-blue-400' : 
                            recipient.role === 'vendor' ? 'bg-green-400' :
                            recipient.role === 'broker' ? 'bg-purple-400' :
                            'bg-gray-400'
                          }`}></span>
