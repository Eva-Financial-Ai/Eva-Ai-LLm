...
const TestDiagnostics: React.FC<TestDiagnosticsProps> = ({ showFullDetails = false }) => {
  const { transactions, currentTransaction, fetchTransactions, loading, error } = useTransactionStore();
    ...
    <div className="p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <div className="p-4 bg-green-50 rounded-lg border border-green-100">
          ...
          <div className="text-3xl font-bold text-green-600">{riskAssessmentTransactions.length}</div>
        </div>
      </div>
      ...
      {currentTransaction ? (
        <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="text-sm text-gray-900">${currentTransaction.amount.toLocaleString()}</div>
          </div>
      ...
      {riskAssessmentTransactions.length > 0 ? (
        <div className="space-y-4">
          {riskAssessmentTransactions.map(transaction => (
            <div key={transaction.id} className="p-4 bg-green-50 rounded-lg border border-green-100">
              <div className="grid grid-cols-2 gap-2">
                <div className="text-sm font-medium text-gray-500">ID</div>
