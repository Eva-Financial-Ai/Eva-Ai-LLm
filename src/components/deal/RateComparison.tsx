...
const RateComparison: React.FC<RateComparisonProps> = ({ isOpen, onClose }) => {
  ...
  const [selectedLenders, setSelectedLenders] = useState<string[]>(['lender1', 'lender2', 'lender3']);
  ...
  const getEffectiveRate = (lender: LenderRate): number => {
    ...
    const termOptions = Object.keys(lender.termAdjustments).map(Number).sort((a, b) => a - b);
    ...
    const downPaymentOptions = Object.keys(lender.downPaymentAdjustments).map(Number).sort((a, b) => a - b);
  ...
  const calculatePayment = (rate: number): number => {
    ...
    if (monthlyRate === 0) {
      ...
      return (amountFinanced * monthlyRate * Math.pow(1 + monthlyRate, term)) / 
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="mt-6">
                <label className="block text-sm font-medium text-gray-700 mb-2">Select Lenders</label>
            ...
            <div className="md:col-span-2">
              ...
              {comparisons.length === 0 ? (
                <div className="bg-gray-50 p-4 rounded-lg text-center">
                  <p className="text-gray-600">Please select at least one lender to compare rates.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {/* Rate Comparison Chart */}
                  <div className="bg-white p-4 rounded-lg border border-gray-200">
                    <h4 className="text-sm font-medium text-gray-700 uppercase mb-3">Interest Rate Comparison</h4>
                    <div className="space-y-3">
                      {comparisons.map(({ lender, rate }) => (
                        <div key={lender.id} className="flex items-center">
                          ...
                          <div className="w-16 text-right text-sm font-medium">{formatPercent(rate)}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Payment Comparison */}
                  <div className="bg-white p-4 rounded-lg border border-gray-200">
                    <h4 className="text-sm font-medium text-gray-700 uppercase mb-3">Monthly Payment Comparison</h4>
                    <div className="space-y-3">
                        ...
                        <div key={lender.id} className="flex items-center">
                          ...
                          <div className="flex-1 h-6 bg-gray-100 rounded overflow-hidden">
                            <div 
                              ...
                              style={{ width: `${(payment / comparisons[comparisons.length - 1].payment) * 90}%` }}
                            ></div>
                          </div>
                          <div className="w-24 text-right text-sm font-medium">{formatCurrency(payment)}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-4 rounded-lg border border-gray-200 overflow-x-auto">
                    <h4 className="text-sm font-medium text-gray-700 uppercase mb-3">Detailed Comparison</h4>
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead>
                        <tr>
                          <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Lender</th>
                          ...
                          <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Total Interest</th>
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {comparisons.map(({ lender, rate, payment, totalInterest, totalCost }) => (
                          <tr key={lender.id} className="hover:bg-gray-50">
                            <td className="px-3 py-2 whitespace-nowrap text-sm font-medium text-gray-900">{lender.name}</td>
                            <td className="px-3 py-2 whitespace-nowrap text-sm text-right">{formatPercent(rate)}</td>
                            <td className="px-3 py-2 whitespace-nowrap text-sm text-right font-medium">{formatCurrency(payment)}</td>
                            <td className="px-3 py-2 whitespace-nowrap text-sm text-right">{formatCurrency(totalInterest)}</td>
                            <td className="px-3 py-2 whitespace-nowrap text-sm text-right">{formatCurrency(totalCost)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
