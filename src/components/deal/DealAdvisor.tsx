...
const DealAdvisor: React.FC<DealAdvisorProps> = ({ className = '' }) => {
  ...
  useEffect(() => {
    console.log('DealAdvisor component initializing...');
    ...
    const initializeAdvisor = async () => {
      try {
        ...
        if (!currentTransaction) {
          console.log('No transaction data available for DealAdvisor');
          setError('Please select a transaction to access advisor tools');
        } else {
          console.log('Transaction data loaded for DealAdvisor:', currentTransaction.id);
        }
      } catch (err) {
  ...
  const handleSmartMatchingClick = () => {
    ...
    setChatMessages([
      {
        role: 'assistant',
        content: 'Welcome to Smart Matching! I can help you find the perfect financing structure for your needs. Let me ask you a few questions to get started.'
      },
      {
        role: 'assistant',
                      ...
                      if (message) {
                        ...
                        setTimeout(() => {
                          setChatMessages(prev => [...prev, { 
                            role: 'assistant', 
                            content: 'Thank you for sharing that information. Based on your needs, I recommend exploring equipment leasing options with terms between 48-60 months. Would you like me to match you with optimal financing structures?' 
                          }]);
                        }, 1000);
