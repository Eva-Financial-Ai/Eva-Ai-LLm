...
const TermSheetGenerator: React.FC<TermSheetGeneratorProps> = ({
  ...
  // State
  const [step, setStep] = useState<'participants' | 'generating' | 'complete' | 'error'>('participants');
  ...
  // Generate the term sheet and start the workflow
  const handleGenerateTermSheet = async () => {
    try {
      ...
      if (assetSellersCount > 0 && participants.assetSellers?.length) {
        completeParticipants.assetSellers = participants.assetSellers.filter(s => s?.name && s?.email);
      }
      ...
      <div className="space-y-6">
        ...
        <div className="border border-gray-200 rounded-lg p-4">
          ...
          {Array.from({ length: guarantorsCount }).map((_, index) => (
            <div key={`guarantor-${index}`} className="mb-4 last:mb-0 border-t border-gray-100 pt-4 first:border-t-0 first:pt-0">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label htmlFor={`guarantor-${index}-name`} className="block text-sm font-medium text-gray-700">
                    Name
                  </label>
                  <input
                    type="text"
                    ...
                    onChange={(e) => handleParticipantChange('guarantors', 'name', e.target.value, index)}
                    className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm"
                    required
                  />
                </div>
                <div>
                  <label htmlFor={`guarantor-${index}-email`} className="block text-sm font-medium text-gray-700">
                    Email
                  </label>
                  <input
                    type="email"
                    ...
                    onChange={(e) => handleParticipantChange('guarantors', 'email', e.target.value, index)}
                    className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm"
                    required
                  />
                </div>
        ...
        <div className="border border-gray-200 rounded-lg p-4">
          ...
          {Array.from({ length: assetSellersCount }).map((_, index) => (
            <div key={`seller-${index}`} className="mb-4 last:mb-0 border-t border-gray-100 pt-4 first:border-t-0 first:pt-0">
              <div className="flex justify-between items-center mb-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label htmlFor={`seller-${index}-name`} className="block text-sm font-medium text-gray-700">
                    Name
                  </label>
                  <input
                    type="text"
                    ...
                    onChange={(e) => handleParticipantChange('assetSellers', 'name', e.target.value, index)}
                    className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm"
                    required
                  />
                </div>
                <div>
                  <label htmlFor={`seller-${index}-email`} className="block text-sm font-medium text-gray-700">
                    Email
                  </label>
                  <input
                    type="email"
                    ...
                    onChange={(e) => handleParticipantChange('assetSellers', 'email', e.target.value, index)}
                    className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm"
                    required
                  />
                </div>
          ...
          {Object.entries(processingSteps).map(([key, step]) => (
            <div key={key} className="flex items-center">
              <div className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center
                ...
                {step.status === 'pending' && (
                  <span className="text-gray-500">{Object.keys(processingSteps).indexOf(key) + 1}</span>
                )}
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full lg:max-w-2xl">
          ...
          <div className="bg-primary-700 px-4 py-3 sm:px-6 flex justify-between items-center">
            <h3 className="text-lg leading-6 font-medium text-white">
              {step === 'participants' ? 'Generate Term Sheet' : 
               step === 'generating' ? 'Processing' :
               step === 'complete' ? 'Complete' : 'Error'}
            </h3>
            {step !== 'generating' && (
              <button
                type="button"
