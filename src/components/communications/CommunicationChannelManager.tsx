...
const CommunicationChannelManager: React.FC<CommunicationChannelManagerProps> = ({
  ...
  const handleRefreshSync = (channelId: string) => {
    ...
    console.log(`Refreshing sync for channel: ${channelId}`);
        ...
        <div className="flex overflow-x-auto pb-2 mb-2 border-b">
          {channels.map(channel => (
            ...
            >
              ...
              {!channel.isConnected && (
                <span className="ml-1 h-2 w-2 rounded-full bg-gray-400" title="Not connected"></span>
              )}
            </button>
          ))}
        </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex flex-col items-end">
                  <div className="flex items-center mb-2">
                    <span className={`h-2 w-2 rounded-full mr-2 ${channel.isConnected ? 'bg-green-500' : 'bg-red-500'}`}></span>
                    <span className="text-sm">{channel.isConnected ? 'Connected' : 'Disconnected'}</span>
                  </div>
