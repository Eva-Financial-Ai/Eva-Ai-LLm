...
const ChatWidget: React.FC<ChatWidgetProps> = ({ 
  ...
  useEffect(() => {
    ...
    if (mode === 'eva') {
      ...
      welcomeMessage = 'Risk Advisor here. I can help identify and mitigate risks in your transaction.';
    } else {
      welcomeMessage = 'Welcome to Clear Communications. How can I assist with your client interactions?';
    }
    
    setMessages([
      { text: welcomeMessage, sender: 'ai', timestamp: new Date() }
    ]);
  }, [mode]);
  ...
  const generateEVAResponse = (query: string) => {
    ...
    } else if (query.toLowerCase().includes('transaction') || query.toLowerCase().includes('loan')) {
      ...
      >
        {isOpen ? (
          ...
          >
            ...
            <div className={`${mode === 'risk' ? 'bg-red-600' : mode === 'communications' ? 'bg-blue-600' : 'bg-primary-600'} px-4 py-3 flex justify-between items-center chat-drag-handle cursor-move`}>
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
              {messages.map((message, index) => (
                <div 
                  key={index} 
                  className={`mb-3 ${message.sender === 'user' ? 'ml-auto' : ''}`}
                >
                  <div className={`p-3 rounded-lg inline-block max-w-xs ${
                    message.sender === 'user' 
