...
class DocumentGenerationService {
  ...
  async generateTermSheet(termSheetData: TermSheetData): Promise<{ fileId: string; fileUrl: string }> {
  ...
  async uploadToFilelock(
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        resolve({
          id: `filelock-${fileId}`,
          securePath: `/filelock/secure/${transactionId}/${fileId}`
        });
      }, 1000);
    });
  }
  ...
  async checkVerificationStatus(
    ...
    console.log(`Checking ${verificationType.toUpperCase()} status for participant:`, participantId);
