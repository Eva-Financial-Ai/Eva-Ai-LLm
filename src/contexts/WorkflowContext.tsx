...
const defaultContext: WorkflowContextType = {
  ...
  generateMetrics: () => ({
    ...
    businessAge: { yearsInBusiness: 0, industryPeerPercentile: 0, stabilityRating: 0, historicalConsistency: 0 },
    industryReputationScore: 0, 
    reputation: { marketPerception: 0, publicSentiment: 0, customerSatisfaction: 0, industryAwards: 0 },
    businessStabilityScore: 0,
    stability: { cashFlowConsistency: 0, revenueGrowth: 0, employeeRetention: 0, marketPositionStrength: 0 }
  }),
...
export const WorkflowProvider = ({ children }: WorkflowProviderProps) => {
        ...
        if (updatedTransaction.timeTracking) {
          ...
          if (prevPath && lastVisitTime) {
            ...
            updatedTransaction.timeTracking.timeMetrics.totalTimeSpent[prevPath] = 
              (updatedTransaction.timeTracking.timeMetrics.totalTimeSpent[prevPath] || 0) + timeSpent;
          }
  ...
  const generateMetrics = useCallback((transactionId: string): Metrics => {
  ...
  const updateTimeTracking = useCallback((transactionId: string) => {
    const transaction = transactions.find(t => t.id === transactionId);
  ...
  const getTimeInStageFormatted = useCallback((transactionIdOrObject: string | Transaction): string => {
    let transaction: Transaction | undefined;
    ...
    const stageStartTime = new Date(transaction.timeTracking.timeMetrics.stageStartTimes[currentStage]).getTime();
    const currentTime = new Date().getTime();
  ...
  const addTransaction = useCallback(async (transactionData: Omit<Transaction, 'id'>): Promise<Transaction | null> => {
    setLoading(true);
    setError(null);
  ...
  const updateTransaction = useCallback(async (updatedTransaction: Transaction): Promise<Transaction | null> => {
  ...
  const advanceStage = useCallback(async (transactionIdOrObject: string | Transaction, stage: WorkflowStage): Promise<void> => {
    setLoading(true);
    setError(null);
    ...
    if (!transaction) {
      console.error(`[WorkflowContext] Cannot advance stage: transaction not found with ID ${transactionId}`);
    ...
    try {
      ...
      if (updatedTransaction.timeTracking) {
        ...
        const stageStartTime = updatedTransaction.timeTracking.timeMetrics.stageStartTimes[prevStage];
        if (stageStartTime) {
          const stageEndTime = now;
          const stageDuration = new Date(stageEndTime).getTime() - new Date(stageStartTime).getTime();
  ...
  const currentStage = useMemo(() => currentTransaction?.currentStage || null, [currentTransaction]);
