...
const generateMockAssets = (): Asset[] => {
  ...
  assetClasses.forEach(assetClass => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const verificationStatus = Math.random() > 0.7 
        ? VerificationStatus.VERIFIED 
        : (Math.random() > 0.5 ? VerificationStatus.PENDING : VerificationStatus.UNVERIFIED);
      ...
      ownership: [
        {
            owner: getOwnerName(),
          percentage: 100,
            since: new Date(new Date().setFullYear(new Date().getFullYear() - Math.floor(Math.random() * 3))).toISOString()
          }
        ],
      ...
      trackingInfo: {
        ...
        blockchainVerification: {
          ...
          transactionHash: verificationStatus !== VerificationStatus.UNVERIFIED 
            ? `0x${Math.random().toString(36).substring(2, 38)}` 
            : undefined,
          verificationDate: verificationStatus === VerificationStatus.VERIFIED 
            ? getRandomDate() 
            : undefined,
        ...
        dateCreated: new Date(new Date().setMonth(new Date().getMonth() - Math.floor(Math.random() * 12))).toISOString()
      });
    }
  });
...
const getAssetName = (assetClass: AssetClass): string => {
  switch (assetClass) {
    case AssetClass.CASH_EQUIVALENTS:
      return ['High-Yield Savings', 'Money Market Fund', 'T-Bill Portfolio'][Math.floor(Math.random() * 3)];
    case AssetClass.COMMERCIAL_PAPER_SECURED:
      return ['Secured Commercial Paper', 'Asset-Backed CP', 'Collateralized Notes'][Math.floor(Math.random() * 3)];
    case AssetClass.GOVERNMENT_BONDS:
      return ['Treasury Bond Portfolio', 'Municipal Bond Fund', 'Government Securities'][Math.floor(Math.random() * 3)];
    case AssetClass.CORPORATE_BONDS:
      return ['Corporate Bond Portfolio', 'Investment Grade Bonds', 'High-Yield Bond Fund'][Math.floor(Math.random() * 3)];
    case AssetClass.EQUITIES:
      return ['Blue Chip Stock Portfolio', 'Growth Stock Fund', 'Dividend Stock Holdings'][Math.floor(Math.random() * 3)];
    case AssetClass.MUTUAL_FUNDS:
      return ['Vanguard Index Fund', 'Fidelity Growth Fund', 'Balanced ETF Portfolio'][Math.floor(Math.random() * 3)];
    case AssetClass.REAL_ESTATE:
      return ['Commercial Property', 'Residential Apartments', 'Industrial Complex'][Math.floor(Math.random() * 3)];
    case AssetClass.COMMODITIES:
      return ['Gold Holdings', 'Agricultural Futures', 'Oil & Gas Reserves'][Math.floor(Math.random() * 3)];
    case AssetClass.CRYPTO:
      return ['Bitcoin Holdings', 'Ethereum Portfolio', 'Diversified Crypto Fund'][Math.floor(Math.random() * 3)];
    case AssetClass.DERIVATIVES:
      return ['Options Portfolio', 'Futures Contracts', 'Interest Rate Swaps'][Math.floor(Math.random() * 3)];
    case AssetClass.PRIVATE_EQUITY:
      return ['Venture Capital Fund', 'Private Equity Stake', 'Startup Investment'][Math.floor(Math.random() * 3)];
    case AssetClass.FOREX:
      return ['Foreign Currency Holdings', 'Multi-Currency Account', 'FOREX Trading Account'][Math.floor(Math.random() * 3)];
    case AssetClass.EQUIPMENT:
      return ['Manufacturing Equipment', 'Industrial Machinery', 'Medical Equipment'][Math.floor(Math.random() * 3)];
    case AssetClass.VEHICLES:
      return ['Commercial Fleet', 'Aircraft Lease', 'Heavy Equipment Fleet'][Math.floor(Math.random() * 3)];
    case AssetClass.UNSECURED_COMMERCIAL_PAPER:
      return ['Unsecured CP Portfolio', 'Short-Term Notes', 'Corporate Promissory Notes'][Math.floor(Math.random() * 3)];
    case AssetClass.INTELLECTUAL_PROPERTY:
      return ['Patent Portfolio', 'Trademark Rights', 'Copyright Holdings'][Math.floor(Math.random() * 3)];
    case AssetClass.DIGITAL_ASSETS:
      return ['Digital Media Rights', 'SaaS License Portfolio', 'NFT Collection'][Math.floor(Math.random() * 3)];
    default:
...
const getAssetSubclass = (assetClass: AssetClass): string => {
  switch (assetClass) {
    case AssetClass.REAL_ESTATE:
      return ['Commercial', 'Residential', 'Industrial', 'Mixed-Use'][Math.floor(Math.random() * 4)];
    case AssetClass.EQUIPMENT:
...
export const PortfolioNavigatorPage: React.FC = () => {
  ...
  const handleSettingChange = (setting: string, value: any) => {
    if (setting.includes('.')) {
      ...
      setPortfolioSettings({
        ...portfolioSettings,
        [parent]: {
          ...(portfolioSettings[parent as keyof typeof portfolioSettings] as Record<string, any> || {}),
          [child]: value
        }
      });
    } else {
  ...
  const portfolioSummary = useMemo(() => {
    ...
    const verified = mockAssets.filter(a => a.blockchainVerification.status === VerificationStatus.VERIFIED);
    const pending = mockAssets.filter(a => a.blockchainVerification.status === VerificationStatus.PENDING);
    const unverified = mockAssets.filter(a => a.blockchainVerification.status === VerificationStatus.UNVERIFIED);
  ...
  const performanceMetrics = useMemo(() => {
    ...
    const averagePerformance = mockAssets.reduce((sum, asset) => sum + (asset.performance || 0), 0) / mockAssets.length;
          ...
          <div className="flex">
            ...
            <div className="ml-3 flex-1 md:flex md:justify-between">
              <p className="text-sm text-blue-700">
                ...
                {portfolioRole === PortfolioUserRole.OWNER && ' You have full access to manage and verify all assets.'}
                {portfolioRole === PortfolioUserRole.MANAGER && ' You can manage portfolio composition and initiate verification.'}
                {portfolioRole === PortfolioUserRole.SERVICER && ' You can view and verify serviceable assets.'}
                {portfolioRole === PortfolioUserRole.VENDOR && ' You have limited view access to certain asset classes.'}
              </p>
            </div>
          </div>
        </div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                ...
                <p className="text-xs text-gray-500 mt-1">{portfolioSummary.assetCount} total assets</p>
              </div>
              
              <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                <h3 className="text-sm font-medium text-gray-500">Verified Assets</h3>
                <p className="text-2xl font-bold text-green-600">{formatCurrency(portfolioSummary.verifiedValue)}</p>
                <p className="text-xs text-gray-500 mt-1">{portfolioSummary.verifiedCount} assets on Shield Ledger</p>
              </div>
              
              <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                <h3 className="text-sm font-medium text-gray-500">Pending Verification</h3>
                <p className="text-2xl font-bold text-yellow-600">{formatCurrency(portfolioSummary.pendingValue)}</p>
                <p className="text-xs text-gray-500 mt-1">{portfolioSummary.pendingCount} assets in process</p>
              </div>
              
              <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                <h3 className="text-sm font-medium text-gray-500">Unverified Assets</h3>
                <p className="text-2xl font-bold text-gray-600">{formatCurrency(portfolioSummary.unverifiedValue)}</p>
                <p className="text-xs text-gray-500 mt-1">{portfolioSummary.unverifiedCount} assets need verification</p>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                ...
                <p className={`text-2xl font-bold ${performanceMetrics.averagePerformance > 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {performanceMetrics.averagePerformance > 0 ? '+' : ''}{performanceMetrics.averagePerformance.toFixed(2)}%
                </p>
              ...
              <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                ...
                <p className="text-2xl font-bold text-green-600">{performanceMetrics.positivePerformerCount}</p>
                <p className="text-xs text-gray-500 mt-1">{performanceMetrics.positivePerformerPercentage.toFixed(1)}% of portfolio</p>
              </div>
              
              <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                <h3 className="text-sm font-medium text-gray-500">Verification Rate</h3>
                <p className="text-2xl font-bold text-blue-600">
                  {((portfolioSummary.verifiedCount / portfolioSummary.assetCount) * 100).toFixed(1)}%
                </p>
            ...
            <div className="mt-8 p-6 bg-gray-50 rounded-lg text-center">
              ...
              <p className="mt-1 text-sm text-gray-500">Our team is building more powerful analytics tools for your portfolio.</p>
              <button
                onClick={() => setShowNewAssetModal(true)}
        ...
        {activeTab === 'settings' && (
          <div className="space-y-6">
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              ...
              <p className="text-gray-600 mb-4">Configure your portfolio settings and preferences.</p>
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      onChange={(e) => handleSettingChange('targetReturn', parseFloat(e.target.value))}
                    />
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) => handleSettingChange('notifications.riskLevelChanges', e.target.checked)}
                  />
          ...
          <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    ...
                    <div className="mt-4 space-y-4">
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm font-medium">{selectedAsset.assetClass.replace('_', ' ')}</span>
                      </div>
                      
                      <div className="flex justify-between">
                        <span className="text-sm text-gray-500">Market Value:</span>
                        <span className="text-sm font-medium">{formatCurrency(selectedAsset.financialData.marketValue)}</span>
                      </div>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className={`text-sm font-medium ${
                          ...
                          {selectedAsset.performance 
                            ? (selectedAsset.performance > 0 ? '+' : '') + selectedAsset.performance.toFixed(2) + '%' 
                            : 'N/A'}
                        </span>
                    </div>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm font-medium">{new Date(selectedAsset.dateCreated).toLocaleDateString()}</span>
                      </div>
                      
                      {selectedAsset.ownership.length > 0 && (
                        <div className="flex justify-between">
                          <span className="text-sm text-gray-500">Primary Owner:</span>
                          <span className="text-sm font-medium">{selectedAsset.ownership[0].owner}</span>
                        </div>
                      )}
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                {selectedAsset.blockchainVerification.status !== VerificationStatus.VERIFIED && portfolioRole !== PortfolioUserRole.VENDOR && (
                  <button 
                    type="button" 
