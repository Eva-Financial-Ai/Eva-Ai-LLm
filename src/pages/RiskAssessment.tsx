...
const RiskAssessment: React.FC = () => {
  ...
  const [assessmentView, setAssessmentView] = useState<'standard' | 'riskmap' | 'configuration'>('standard');
  const [metrics, setMetrics] = useState<Metrics | null>(null);
  const [showModal, setShowModal] = useState(false);
  const [modalType, setModalType] = useState<'humanInLoop' | 'hardDecline' | 'softDecline' | null>(null);
  ...
  const generateMetrics = (transactionId: string): Metrics => {
    ...
    const range = (min: number, max: number, seed: number) =>
      min + (seed % (max - min + 1));
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const riskTransaction = transactions.find((t: { currentStage: string; }) => t.currentStage === 'risk_assessment');
        if (riskTransaction) {
          console.log('Risk Assessment: Found cached transaction in risk_assessment stage:', riskTransaction);
        ...
        const freshRiskTransaction = transactions.find((t: { currentStage: string; }) => t.currentStage === 'risk_assessment');
        
        if (freshRiskTransaction) {
          console.log('Risk Assessment: Found transaction in risk_assessment stage:', freshRiskTransaction);
          ...
          setError("No transactions in risk assessment stage. Please create or advance a transaction to risk assessment.");
        }
      } catch (err) {
  ...
  const submitHumanInLoopRequest = async (data: HumanInLoopFormData, transactionId: string, userId: string): Promise<boolean> => {
    try {
  ...
  const submitHardDecline = async (data: HardDeclineFormData, transactionId: string, userId: string): Promise<boolean> => {
    try {
  ...
  const submitSoftDecline = async (data: SoftDeclineFormData, transactionId: string, userId: string): Promise<boolean> => {
    try {
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="mb-6">
        ...
        <div className="inline-flex rounded-md shadow-sm w-full sm:w-auto" role="group">
          ...
          <button
            type="button"
            onClick={() => setAssessmentView('riskmap')}
            className={`flex-1 sm:flex-auto px-3 py-2 text-sm font-medium border ${
              assessmentView === 'riskmap'
                ? 'bg-primary-600 text-white border-primary-600'
                : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
            }`}
          >
          ...
          <button
            type="button"
            onClick={() => setAssessmentView('configuration')}
            className={`flex-1 sm:flex-auto px-3 py-2 text-sm font-medium border ${
              assessmentView === 'configuration'
                ? 'bg-primary-600 text-white border-primary-600'
                : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
            } rounded-r-md`}
          >
        ...
        <div className="lg:col-span-2">
          ...
          {assessmentView === 'standard' && (
            <div className="bg-white shadow rounded-lg overflow-hidden">
              <div className="p-6">
                {renderMetricsAccordions()}
              </div>
            </div>
          )}
          
          {assessmentView === 'riskmap' && (
            <RiskMapEvaReport transactionId={currentTransaction?.id || ''} />
          )}
          ...
          <div className="bg-white shadow rounded-lg p-4 md:p-6">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium">{currentTransaction?.applicantData?.name || 'N/A'}</p>
              </div>
              ...
              <div>
                ...
                <p className="text-sm font-medium">${(currentTransaction?.amount || 0).toLocaleString()}</p>
              </div>
              ...
              <div className="pt-4 space-y-3">
                <button
                  onClick={() => {
                    ...
                    navigate('/deal-structuring', { state: { transactionId: currentTransaction?.id } });
                  }}
            ...
            <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg p-6 max-w-lg w-full">
                ...
                {modalType === 'humanInLoop' && (
                  <div className="mb-4">
                    <p className="text-sm text-gray-500 mb-4">
                      Request an account manager or processor to contact the applicant for additional information.
                    </p>
                    
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Reason for Call</label>
                        <select 
                          className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500"
                          value={humanInLoopForm.reason}
                          onChange={(e) => setHumanInLoopForm(prev => ({ ...prev, reason: e.target.value }))}
                        >
                          ...
                          <option value="explain_requirements">Explain Documentation Requirements</option>
                          <option value="other">Other Reason</option>
                        </select>
                      </div>
                      
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Notes for Account Manager</label>
                        <textarea 
                          rows={3}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500"
                          placeholder="Provide details about what the account manager should discuss..."
                          value={humanInLoopForm.notes}
                          onChange={(e) => setHumanInLoopForm(prev => ({ ...prev, notes: e.target.value }))}
                        ></textarea>
                      </div>
                        ...
                        <div className="flex space-x-4">
                          <label className="inline-flex items-center">
                            <input 
                              ...
                              onChange={() => setHumanInLoopForm(prev => ({ ...prev, priority: 'low' }))}
                            />
                          ...
                          <label className="inline-flex items-center">
                            <input 
                              ...
                              onChange={() => setHumanInLoopForm(prev => ({ ...prev, priority: 'medium' }))}
                            />
                          ...
                          <label className="inline-flex items-center">
                            <input 
                              ...
                              onChange={() => setHumanInLoopForm(prev => ({ ...prev, priority: 'high' }))}
                            />
                ...
                {modalType === 'hardDecline' && (
                  <div className="mb-4">
                    <p className="text-sm text-gray-500 mb-4">
                      Provide a reason for the hard decline. Hard declines cannot be overcome and will require a new application.
                    </p>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <select 
                          ...
                          onChange={(e) => setHardDeclineForm(prev => ({ ...prev, reason: e.target.value }))}
                        >
                          <option value="credit_score">Insufficient Credit Score</option>
                          <option value="identity_verification">Failed Identity Verification</option>
                      ...
                      <div>
                        ...
                        <textarea 
                          ...
                          onChange={(e) => setHardDeclineForm(prev => ({ ...prev, comments: e.target.value }))}
                        ></textarea>
                      </div>
                    </div>
                  </div>
                ...
                {modalType === 'softDecline' && (
                  <div className="mb-4">
                    <p className="text-sm text-gray-500 mb-4">
                      Provide a reason for the soft decline and steps the applicant can take to overcome the issue.
                    </p>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <select 
                          ...
                          onChange={(e) => setSoftDeclineForm(prev => ({ ...prev, reason: e.target.value }))}
                        >
                          <option value="missing_documents">Missing Documentation</option>
                          <option value="insufficient_income">Insufficient Income Verification</option>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Steps to Overcome</label>
                        <textarea 
                          rows={3}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500"
                          placeholder="Describe what the applicant can do to overcome this issue..."
                          value={softDeclineForm.stepsToOvercome}
                          onChange={(e) => setSoftDeclineForm(prev => ({ ...prev, stepsToOvercome: e.target.value }))}
                        ></textarea>
                      </div>
                      
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Reconsideration Timeline</label>
                        <select 
                          className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500"
                          value={softDeclineForm.reconsiderationTimeline}
                          onChange={(e) => setSoftDeclineForm(prev => ({ ...prev, reconsiderationTimeline: e.target.value }))}
                        >
                ...
                <div className="flex justify-end space-x-3">
                  ...
                  >
                    {isSubmitting ? (
                      ...
                      modalType === 'humanInLoop' ? 'Request Call' : 'Submit'
                    )}
                  </button>
                </div>
              </div>
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <span className="text-sm font-medium">
                            {request.type === 'humanInLoop' ? 'Account Manager Call' : 
                             request.type === 'hardDecline' ? 'Hard Decline' : 'Soft Decline'}
                          </span>
                        ...
                        <span className={`px-2 py-1 text-xs rounded-full ${
                          ...
                          {request.status === 'pending' ? 'Pending' :
                           request.status === 'inProgress' ? 'In Progress' : 'Completed'}
                        </span>
                      </div>
                      
                      {request.type === 'humanInLoop' && (
                        <div className="text-xs text-gray-600">
                          <p><span className="font-medium">Reason:</span> {(request.data as HumanInLoopFormData).reason.replace(/_/g, ' ')}</p>
                          <p><span className="font-medium">Priority:</span> {(request.data as HumanInLoopFormData).priority}</p>
                        </div>
                      )}
          ...
          <div className="bg-white shadow rounded-lg p-4 md:p-6">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm font-medium capitalize">{currentTransaction.currentStage.replace(/_/g, ' ')}</p>
              </div>
