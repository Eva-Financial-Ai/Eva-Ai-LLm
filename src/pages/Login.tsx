...
const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSignUp) {
      ...
      try {
        ...
        setTimeout(() => {
          if (onLoginSuccess) {
            onLoginSuccess({ name: `${formData.firstName} ${formData.lastName}`, email: formData.email });
          }
          navigate('/');
          setLoading(false);
        }, 1000);
      ...
      try {
        ...
        if (response.success) {
          if (onLoginSuccess) {
            onLoginSuccess({ name: response.user?.name, email: formData.email, id: response.user?.id });
          }
          navigate('/');
        } else {
  ...
  const handleAutoLogin = async () => {
    ...
    try {
      ...
      if (response.success) {
        if (onLoginSuccess) {
          onLoginSuccess({ name: response.user?.name, email: 'test@example.com', id: response.user?.id });
        }
        navigate('/');
      } else {
        ...
        <div className="bg-white p-8 shadow rounded-lg">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            {isSignUp && (
              <>
                <div>
                  <label htmlFor="businessAffiliation" className="block text-sm font-medium text-gray-700">
                ...
                <div className="flex items-start">
                  ...
                  <div className="ml-3 text-sm">
                    <label htmlFor="agreeToTerms" className="text-gray-500">
                      By submitting this form and entering your information above, you agree to receive
                      automated text messages from EVA and agree to our <a href="#" className="text-blue-600">Terms of Use and Privacy Policy</a>.
                ...
                <div className="flex items-start">
                  ...
                  <div className="ml-3 text-sm">
                    <label htmlFor="receiveUpdates" className="text-gray-500">
                      Register me for email and SMS notifications regarding latest updates and changes in
                      my request.
                    </label>
                  </div>
                </div>
          ...
          {isSignUp && (
            <div className="mt-4 text-center">
              ...
              <div className="mt-4 flex justify-center space-x-4">
                ...
                <button className="p-2 bg-white border border-gray-300 rounded-full">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14h2v2h-2v-2zm0-10h2v8h-2V6z" fill="#000"/>
                  </svg>
                </button>
                <button className="p-2 bg-white border border-gray-300 rounded-full">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                    <path d="M8 2C4.691 2 2 4.691 2 8v8c0 3.309 2.691 6 6 6h8c3.309 0 6-2.691 6-6V8c0-3.309-2.691-6-6-6H8zm0 2h8c2.206 0 4 1.794 4 4v8c0 2.206-1.794 4-4 4H8c-2.206 0-4-1.794-4-4V8c0-2.206 1.794-4 4-4zm9 1a1 1 0 00-1 1 1 1 0 001 1 1 1 0 001-1 1 1 0 00-1-1zm-5 1c-3.309 0-6 2.691-6 6s2.691 6 6 6 6-2.691 6-6-2.691-6-6-6zm0 2c2.206 0 4 1.794 4 4s-1.794 4-4 4-4-1.794-4-4 1.794-4 4-4z" fill="#0072c6"/>
                  </svg>
                </button>
              </div>
            </div>
